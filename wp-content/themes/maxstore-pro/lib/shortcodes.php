	...
	if ( $the_query->have_posts() ) :
		while ( $the_query->have_posts() ) : $the_query->the_post();
			...
			if ( has_post_thumbnail() ) {
				$return_string .= '<a href="' . get_the_permalink() . '" rel="bookmark">' . get_the_post_thumbnail( $post->ID, 'maxstore-single' ) . '</a>';
			...
			$return_string .= '</div>';
			$return_string .= '<a class="btn btn-primary btn-md outline" href="' . get_the_permalink() . '" itemprop="interactionCount">' . esc_html__( 'Read more', 'maxstore' ) . '</a>';
...
if ( class_exists( 'WooCommerce' ) ) {
	...
	function twp_pro_posts_carousel_shortcode( $atts, $content = null ) {
		...
		if ( $the_query->have_posts() ) :
			while ( $the_query->have_posts() ) : $the_query->the_post();
				...
				if ( has_post_thumbnail() ) {
					$return_string .= '<a href="' . get_the_permalink() . '" rel="bookmark">' . get_the_post_thumbnail( $post->ID, 'maxstore-recent' ) . '</a>';
					$return_string .= '<div class="single-meta-date">';
					$return_string .= '<div class="day">' . get_the_time( 'd' ) . '</div>';
					$return_string .= '<div class="month">' . get_the_time( 'M' ) . '</div>';
					$return_string .= '</div>';
	...
	function twp_pro_category_carousel_shortcode( $atts, $content = null ) {
		...
		if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			...
			foreach ( $terms as $term ) {
				...
				echo apply_filters( 'woocommerce_subcategory_count_html', ' <mark class="count">' . $term_count . __( ' Products', 'maxstore' ) . '</mark>', $term );
				echo '</a>';
				echo '</li>';
	...
	function twp_pro_custom_category_shortcode( $atts, $content = null ) {
		...
		if ( empty( $cat ) || empty( $term ) ) { //check if category exists
			return;
		}
		$term_name		 = $term->name;
					...
					<div class="slider-img col-sm-4">    
						<a href="<?php echo the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
							<div class="slider-grid-img">
								...
								<?php
								if ( has_post_thumbnail() )
									echo the_post_thumbnail( 'maxstore-category' );
								else
									echo '<img src="' . woocommerce_placeholder_img_src() . '" alt="Placeholder" width="350px" height="350px" />';
								?>
							</div>                   
	...
	function twp_pro_custom_category_carousel_shortcode( $atts, $content = null ) {
		...
		$category_link	 = get_term_link( $term );
		$thumb_id		 = get_term_meta( $term_id, 'thumbnail_id', true );
		$term_img		 = wp_get_attachment_image( $thumb_id, 'maxstore-carousel' );
		global $post;
