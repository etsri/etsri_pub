...
if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
	...
	echo do_shortcode( '
		...
		[product-carousel per_page="10" columns="5"]
		[row]
		[col size="3"][image img="' . get_template_directory_uri() . '/template-parts/demo-img/left-banner.jpg" animation="zoomin" ][/col]
		[col size="6"][image img="' . get_template_directory_uri() . '/template-parts/demo-img/top-banner.jpg" animation="zoomout" ][image img="' . get_template_directory_uri() . '/template-parts/demo-img/bottom-banner.jpg" animation="zoomout" title="" size="h2" color="#000"][/col]
	...
	<?php echo do_shortcode( '
		...
		[posts-carousel columns="2" excerpt="30"]
		[/col]
		[/row]
	' ); 
} else {
	esc_html_e( 'No product categories found', 'alpha-store' );
}	
?>
